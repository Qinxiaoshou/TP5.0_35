...
//            return '添加comm成功';     
            ...
            // 关联查询
//                $user  = Rigest::get(1); // Rigest::get(1,'comm');
//                $comm = $user->comm;
//                dump($comm);        
...
//            echo "评论id: {$comm->comment_id} 用户评论内容: {$comm->content}<br/>";             
            ...
            // 根据关联数据来查询当前模型数据
            // 查询有评论过的用户列表
            //$user = Rigest::has('comm')->select();
            // 查询评论过2次以上的用户
            //$user = Rigest::has('comm', '>=', 2)->select();
            // 查询评论内容含有 ThinkPHP5快速入门的用户
            //$user = Rigest::hasWhere('comm', ['content' => 'ThinkPHP5视频教程'])->select();
...
//        }   
        ...
        // 关联查询 
//            $user = Rigest::get(2600); // $user = Rigest::get(2600,'car');
//            echo $user->email . '<br/>';
//            echo $user->nickname . '<br/>';
...
//        $data = input('post.');
//        // 数据验证
//        $result = $this->validate($data,'AdminUsers');
//        if (true !== $result) {
//            return $result;
//        }
//        $users = new Rigest;
...
//        return '用户[ ' . $users->nickname . ':' . $users->user_id . '[phone' . $users->tel . ']新增成功';
    }    
      //  单独验证 某 字段
//    public function add()
//    {

...
//        // 建议的读取Cookie数据的方法是通过Request请求对象的cookie方法
        echo 'request读取Cookie: '.$this->request->cookie('user_name');
        echo "<br/>";
...
//        // 清除
    ...
    public function test34(){   //运用extend/org/util里面的ArrayList
        $list       = ['thinkphp', 'thinkphp5', 'TPshop'];
        $arrayList  = new ArrayList($list);
        $arrayList->add('kancloud');
        $arrayList->remove('2');
        echo  $arrayList->asort();
        dump($arrayList->toArray());
        echo $arrayList->toJson();
    }
    public function test33(){  //运行common.php里面的全局公用的函数my_fun()函数
        echo my_fun();
    }
...
////        $list = Rigest::scope('email,level')->all();
////        print_r($list);
//        
//        $list = Rigest::scope('email') // 可带参数 ,'132456'
//        ->scope('level')
...
//          $user->save();
        ...
        //Rigest::create(['email'=>'1324567@798.com']);  自动完成
    ...
    public function testModel(){
          //  $news = News::get(1);
          // //huizhou.test
          //  $news->source = ['unusual米'=>11];
          //  echo $news->hits.'<br/>'.$news->author; //类型转换
          //  $news->save();
...
//        if ($result = Rigest::create($userArr))
...
//            $user = Rigest::getByMobile('123456');
//            echo $user['mobile'];
//            echo "<br/>";
//            echo $user['email'];

////           如果不是根据主键查询，可以传入数组作为查询条件
//            $user = Rigest::get(['mobile'=>'13554784574','email'=>'511482696@qq.com']);
//            $user = Rigest::where('mobile','13554784574')->find();
//            $user = Rigest::where(['mobile'=>'13554784574','email'=>'511482696@qq.com'])->find();
//            echo $user['mobile'];
//            echo "<br/>";
//            echo $user['email'];      
...
//                  Rigest::update($userArr,['user_id'=>1]);
//                删除操作                    
//                  $user = Rigest::get(2593);
//                  $user->delete();         
//                  // 或者使用
//                  Rigest::destroy(2592);
    }
...
//            改造后     
    ...
    //练习使用
    public function HelloWorldN($name = '( ఠൠఠ )ﾉ'){

            echo "{$name}"."HelloWorldN";
            echo "<br/>";
            print_r($this->request->param());
    }
    public function todayN($year,$month){
            echo "今天是$year 年$month 月";
    }

    public function tsetcheckarea(){
        trace('这是测试调试信息');
        trace([1,2,3]);
        dump('测试');
        dump(['a', 'b', 'c']);
        dump(News::get());
        $list = News::scope('author,hits')->all();
        print_r($list);
    }


    //注册admin用户
    public function  rigestadmin(){
        $username =    $this->request->param('nickname');
        if(empty($username))
            echo "昵称不能为空";
